...
public class VerificationManager {
    ...
    public void startVerification(ServerPlayerEntity player, String discordInput) {
        String discordId = resolveDiscordUser(discordInput);
        ...
        User user = DiscordVerifierMod.jda.retrieveUserById(discordId).complete();
        user.openPrivateChannel().flatMap(ch ->
            ch.sendMessage("Reply **only** with this 6‑digit code to link your account:\n`" + code + "`")
        ).queue();
    }
...
public boolean verifyCode(String code) {
    ...
    DiscordVerifierMod.guild
            .addRoleToMember(
                    UserSnowflake.fromId(p.discordId()),      // member
                    DiscordVerifierMod.verifiedRole             // role
            )
            .queue();
    ...
    private String resolveDiscordUser(String input) {
        ...
        return DiscordVerifierMod.guild.getMembersByName(parts[0], true).stream()
